...
const EMAILJS_PUBLIC_KEY = "4MVXaPaxaZg-lQnlm";
...
function buildReport(data) {
  ...
  const decColor = { go: "#22C55E", cond: "#F59E0B", stop: "#EF4444" }[conscience];
  ...
  const nokItems = [];
  RISK_CATEGORIES.forEach(cat => { cat.checks.forEach((ck, i) => { if (checks[`${cat.id}-${i}`] === "nok") nokItems.push(ck); }); });

  const rows = RISK_CATEGORIES.map(cat => cat.checks.map((ck, i) => {
    const v = checks[`${cat.id}-${i}`];
    const c = v === "ok" ? "#22C55E" : v === "nok" ? "#EF4444" : "#999";
    const l = v === "ok" ? "OK" : v === "nok" ? "NOK" : "N/A";
    return `<tr><td style="padding:5px 8px;color:#555;border-bottom:1px solid #f0f0f0;">${ck}</td><td style="padding:5px 8px;text-align:center;color:${c};font-weight:700;border-bottom:1px solid #f0f0f0;">${l}</td></tr>`;
  }).join("")).join("");

  return `<div style="font-family:Calibri,sans-serif;max-width:600px;margin:0 auto;">
<div style="background:#1A1A2E;padding:16px 20px;">
<div style="color:white;font-size:18px;font-weight:800;letter-spacing:2px;">NEKUTA</div>
<div style="color:#E8611A;font-size:10px;font-weight:700;letter-spacing:3px;">LMRA - ARDM</div>
</div>
<div style="background:${decColor};padding:14px 20px;color:white;font-size:16px;font-weight:800;text-align:center;">${decLabel}</div>
<div style="background:white;padding:20px;">
<table style="width:100%;border-collapse:collapse;margin-bottom:14px;">
<tr><td style="padding:5px 0;color:#888;font-size:12px;width:100px;">Lieu</td><td style="padding:5px 0;font-weight:600;font-size:13px;">${info.lieu}</td></tr>
<tr><td style="padding:5px 0;color:#888;font-size:12px;">Travail</td><td style="padding:5px 0;font-weight:600;font-size:13px;">${info.travail}</td></tr>
<tr><td style="padding:5px 0;color:#888;font-size:12px;">Date</td><td style="padding:5px 0;font-size:13px;">${info.date} - ${info.heure}</td></tr>
<tr><td style="padding:5px 0;color:#888;font-size:12px;">EPI</td><td style="padding:5px 0;font-size:12px;">${epiLabels}</td></tr>
<tr><td style="padding:5px 0;color:#888;font-size:12px;">Signataire(s)</td><td style="padding:5px 0;font-weight:600;font-size:13px;">${signList}</td></tr>
<tr><td style="padding:5px 0;color:#888;font-size:12px;">Horodatage</td><td style="padding:5px 0;font-size:12px;">${ts}</td></tr>
</table>
${nokItems.length > 0 ? `<div style="background:#FEF2F2;border-left:4px solid #EF4444;padding:10px 14px;margin-bottom:14px;">
<div style="color:#EF4444;font-weight:700;font-size:12px;margin-bottom:6px;">POINTS NOK (${nokItems.length})</div>
${nokItems.map(item => `<div style="color:#666;font-size:12px;padding:2px 0;">- ${item}</div>`).join("")}
</div>` : ""}
${envD.length > 0 || intR.length > 0 ? `<div style="background:#FFFBEB;border-left:4px solid #F59E0B;padding:10px 14px;margin-bottom:14px;">
<div style="color:#F59E0B;font-weight:700;font-size:12px;margin-bottom:6px;">DANGERS ET RISQUES</div>
${envD.length > 0 ? `<div style="font-size:11px;color:#666;">Environnement: ${envD.join(", ")}</div>` : ""}
${intR.length > 0 ? `<div style="font-size:11px;color:#666;margin-top:3px;">Intervention: ${intR.join(", ")}</div>` : ""}
</div>` : ""}
${comment ? `<div style="background:#F0FDF4;border-left:4px solid #22C55E;padding:10px 14px;margin-bottom:14px;">
<div style="color:#22C55E;font-weight:700;font-size:12px;">MESURES</div>
<div style="color:#666;font-size:12px;margin-top:4px;">${comment}</div>
</div>` : ""}
<div style="font-size:12px;font-weight:700;color:#1A1A2E;margin:14px 0 6px;">VERIFICATIONS</div>
<table style="width:100%;border-collapse:collapse;font-size:11px;">
<tr style="background:#f1f5f9;"><th style="text-align:left;padding:5px 8px;color:#888;">Point</th><th style="text-align:center;padding:5px 8px;color:#888;width:50px;">Resultat</th></tr>
${rows}
</table>
</div>
<div style="background:#1A1A2E;padding:12px 20px;text-align:center;">
<div style="color:#E8611A;font-size:9px;font-weight:700;letter-spacing:2px;">NEKUTA - VCA</div>
<div style="color:rgba(255,255,255,0.3);font-size:8px;margin-top:2px;">Pas de securite, pas de travail</div>
</div>
</div>`;
}
