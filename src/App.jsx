...
function buildReport(data) {
  ...
  const nokItems = [];
  const allChecks = [];
  RISK_CATEGORIES.forEach(cat => { cat.checks.forEach((ck, i) => {
    const v = checks[`${cat.id}-${i}`];
    const l = v === "ok" ? "OK" : v === "nok" ? "NOK" : "N/A";
    allChecks.push(`  [${l}] ${ck}`);
    if (v === "nok") nokItems.push(ck);
  }); });

  let r = `========================================\n`;
  r += `  NEKUTA - LMRA DIGITAL (VCA)\n`;
  r += `========================================\n\n`;
  r += `  DECISION: ${decLabel}\n\n`;
  r += `----------------------------------------\n`;
  r += `  IDENTIFICATION\n`;
  r += `----------------------------------------\n`;
  r += `  Lieu:          ${info.lieu}\n`;
  r += `  Travail:       ${info.travail}\n`;
  r += `  Date:          ${info.date} - ${info.heure}\n`;
  r += `  Signataire(s): ${signList}\n`;
  r += `  Horodatage:    ${ts}\n\n`;
  r += `----------------------------------------\n`;
  r += `  EPI SELECTIONNES\n`;
  r += `----------------------------------------\n`;
  ...
  if (nokItems.length > 0) {
    r += `----------------------------------------\n`;
    r += `  POINTS NOK (${nokItems.length})\n`;
    r += `----------------------------------------\n`;
    nokItems.forEach(item => { r += `  [NOK] ${item}\n`; });
    r += `\n`;
  }
  if (envD.length > 0 || intR.length > 0) {
    r += `----------------------------------------\n`;
    r += `  DANGERS ET RISQUES IDENTIFIES\n`;
    r += `----------------------------------------\n`;
    if (envD.length > 0) r += `  Environnement: ${envD.join(", ")}\n`;
    if (intR.length > 0) r += `  Intervention:  ${intR.join(", ")}\n`;
    r += `\n`;
  }
  if (comment) {
    r += `----------------------------------------\n`;
    r += `  MESURES COMPLEMENTAIRES\n`;
    r += `----------------------------------------\n`;
    r += `  ${comment}\n\n`;
  }
  r += `----------------------------------------\n`;
  r += `  DETAIL DES VERIFICATIONS (${allChecks.length})\n`;
  r += `----------------------------------------\n`;
  r += allChecks.join(`\n`) + `\n\n`;
  r += `========================================\n`;
  r += `  Pas de securite, pas de travail\n`;
  r += `  NEKUTA - 33 Bd Leopold II, Bruxelles\n`;
  r += `========================================\n`;
  return r;
}
